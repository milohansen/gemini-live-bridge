...
from audio import ESP_OUTPUT_RATE, GEMINI_INPUT_RATE, GEMINI_OUTPUT_RATE
...
class GeminiSession:
    def __init__(self, address, proxy_server):
    ...
    async def process_incoming_audio(self, raw_audio, source_rate):
        ...
        audio_16k = (
            ...
            if source_rate == GEMINI_INPUT_RATE
                ...
                self.proxy.resample_audio, raw_audio, source_rate, GEMINI_INPUT_RATE
            )
        )
    ...
    async def sender_task(self, session):
        while self.running:
            try:
                chunk = await self.audio_queue_mic.get()
                await session.send_realtime_input(
                    audio={"data": chunk, "mime_type": f"audio/pcm;rate={GEMINI_INPUT_RATE}"}
                )
    ...
    async def receiver_task(self, session):
        while self.running:
            try:
                            ...
                            for part in server_content.model_turn.parts:
                                if part.inline_data:
                                    ...
                                    audio_48k = await asyncio.to_thread(
                                        self.proxy.resample_audio, audio_24k, GEMINI_OUTPUT_RATE, ESP_OUTPUT_RATE
                                    )
